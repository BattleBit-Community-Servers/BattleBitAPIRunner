...
public class SniperTeamLimit : BattleBitModule
{
    ...
    public override async Task OnGameStateChanged(GameState oldState, GameState newState){
        ...
        // On EndingGame state, force Recons to Assault
    ...
    public void ChangeToAssault(RunnerPlayer player){
        if (player.Role == BattleBitAPI.Common.GameRole.Recon){
        ...
        Console.WriteLine($"{timestampString} | {"CTA",8} | Not Recon, Skipping: {player.Role}");
    }
    ...
    {
        Console.WriteLine($"{timestampString} | {"OPCR",8} | {player.ToString()} | Role: {role}");
        //     if(player.Role == GameRole.Recon)
        //         player.SetNewRole(GameRole.Assault);
        if (player.Role == GameRole.Recon){
            string team_name = player.Team.ToString();
            int sniper_count = (team_name == "TeamA") ? team_a_sniper_count : team_b_sniper_count;
            Console.WriteLine($"{timestampString} | {"OPCR",8} | {player.ToString()} is requesting sniper");
            if (sniper_count < team_sniper_limit){
                sniper_count++;
                if (team_name == "TeamA"){
                    team_a_sniper_count = sniper_count;
                    Console.WriteLine($"{timestampString} | {"OPCR",8} | Team A: {team_a_sniper_count}/{team_sniper_limit}");
                }
                else if (team_name == "TeamB"){
                    team_b_sniper_count = sniper_count;
                    Console.WriteLine($"{timestampString} | {"OPCR",8} | Team B: {team_b_sniper_count}/{team_sniper_limit}");
                }
                Console.WriteLine($"{timestampString} | {"OPCR",8} | Team A: {team_a_sniper_count} | Team B: {team_b_sniper_count}");
            }
            else{
                Console.WriteLine($"{timestampString} | {"OPCR",8} | {team_name} is at the sniper limit of {team_sniper_limit}");
                Console.WriteLine($"{timestampString} | {"OPCR",8} | Team A: {team_a_sniper_count} | Team B: {team_b_sniper_count}");
                player.Message($"Only {team_sniper_limit} sniper(s) are allowed per team.\nPlease pick a different class.", 5);
                player.SetNewRole(GameRole.Assault);
            }
        }
        ...
        //     }
        // }
        Console.WriteLine($"{timestampString} | {"OPCR",8} | {player.ToString()} | Role Changed: {role}");
    }
